...
test_that("Trajectory arg not found in initial pop table triggers stop()", {
    dt <- data.table(a = integer(), b = integer(), c = integer())
...
test_that("Trajectory states not found in initial pop table triggers stop()", {
    dt <- data.table(a = integer(), b = integer(), c = integer())
    trj <- new_trajectory(empty_trajectory_fn, c("a"), "a")
    # No error by default when table contains column "a"
    expect_no_error(check_trajectory(trj, dt))
    # Error when states doesn't exist in dt
    trj$states <- "d"
    expect_error(
        check_trajectory(trj, dt),
        "initial population columns do not contain trajectory\\$states",
        info = "states d does not exist in dt"
    )
})
test_that("Multivariate trajectory states not found in initial pop table triggers stop()", {
    dt <- data.table(a = integer(), b = integer(), c = integer())
...
test_that("Hazard attribute of incorrect type triggers stop()", {
    ...
    haz <- new_hazard(empty_hazard_fn, c("age"), trn)
    haz$transitions <- list(
        new_transition(empty_transition_fn, c("age"), "age"),
        12
    )
    expect_error(
        check_hazard(haz),
...
test_that("Transition states not found in initial pop table triggers stop()", {
    dt <- data.table(a = integer(), b = integer(), c = integer())
    trn <- new_transition(empty_transition_fn, c("a"), "a")
    # No error by default when table contains column "a"
...
test_that("History with missing attribute triggers stop()", {
    ...
    for (field in required_fields) {
        hist <- new_history(
            list(new_column("test", empty_reduction_fn, c("a"))),
            1
        )
        hist[[field]] <- NULL
        expect_error(
...
test_that("initPop must inherit from data.frame", {
    ...
    expect_error(
        check_parameters(prm, lst),
        "must inherit from class data.frame",
        info = paste(
            "Matrix passed to check_parameters (via run_simulation()) should error."
        )
    )
    expect_error(
        check_parameters(prm, mat),
        "must inherit from class data.frame",
        info = paste(
            "Matrix passed to check_parameters (via run_simulation()) should error."
        )
