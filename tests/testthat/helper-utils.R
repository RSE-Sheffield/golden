...
#'
#' @param states This dictates the length of the vector returned
#' @return A vector of length. equal to param states, filled with 0.5
empty_hazard_fn <- function(states) {
    return(rep(0.5, length(states)))
}
...
#' All hazards pass
always_hazard_fn <- function(states) {
    return(rep(Inf, length(states)))
}
