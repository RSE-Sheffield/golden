...
# Note, username may change in future if repository is transferred
install_github("RSE-Sheffield/golden")
```
...
TODO CHECK: check & list what objects are allowed for populations (data.frames, matrices?)
...
TODO CHECK: what happens with a function depending only on `~STEP`? Case for allowing version that return scalar and handling in the same way
...
```
TODO CHECK: is there a way to handle the ETA gracefully in outputs like this?
...
str(result0)
```
TODO CHECK: is history `NULL` or empty with no history specified?
...
result1$pop
```
TODO CHECK: probably need a section somewhere on implementation that includes user-relevant things like ordering of updates/hazards etc
...
TODO CHECK: check can use random hazards.
...
TODO CHECK: must be super consistent in use of language around event/hazard etc.
