...
morthaz0 <- new_hazard(
  ...
  transitions = new_transition(  # creates a transition
    ...
    state = "death"              # col transition acts on
  )
)
...
bptc_traj <- new_trajectory(
  ...
  property = c("sbp", "tc")       # outputs (list of vectors from fn)
)
...
hazlist <- list(
  ...
  new_hazard(
    ...
    transitions = new_transition(
      ...
      state = "cvd_count"      # vars affected by transition
    )
  ),
  ## deaths
  new_hazard(
